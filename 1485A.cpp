...
void idharDekh()
{
    ...
    str(b);
    ll m=b.size();
    unordered_map<string,ll>ans1,ans2;
    fr(i,0,n){
        fr(j,i,n){
            ans1[a.substr(i,j-i+1)]=j-i+1;
        }
    }
    fr(i,0,m){
        fr(j,i,m){
            ans2[b.substr(i,j-i+1)]=j-i+1;
        }
    }
    ll ct=m+n;
    for(auto x:ans1){
        if(ans2.find(x.ff)!=ans2.end()){
            ct=min(ct,n+m-x.ss-ans2[x.ff]);
        }
    }
    cout<<ct<<endl;
}
