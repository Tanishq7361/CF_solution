...
#define vin(a, n) vi a(n); for(ll i=0; i<(n); ++i) cin >> a[i];
#define arra(n) ll n; for(ll i=0; i<(n); ++i) cin >> a[i];
#define fo(i,n) for(ll i=0;i<n;i++)
#define rfo(i,n) for(ll i=n-1;i>=0;i--)
#define print(a) cout<<a<<endl;
#define vlll(a,n) vector<long long> a(n); for(ll i=0; i<(n); ++i) cin >> a[i];
...
void solve(){
    ...
    if(a[0][0]!='H'&& a[0][0]!='A'&& a[0][0]!='C'&& a[0][0]!='K'){
        return;
    }    
    vvi b(n,vi(m,0));
    b[0][0]=1;
    fr(i,0,n){
        fr(j,0,m){
            if (a[i][j]!='H'&& a[i][j]!='A'&& a[i][j]!='C'&& a[i][j]!='K'){
                continue;
            }    
            if(i>0){ 
