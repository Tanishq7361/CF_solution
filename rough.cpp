...
void idharDekh()
{
    ...
    ll y = modmult(n,m);
    ll z = min(n, m);
    ll dig =(modmult(2,z) - 3 + MOD)%MOD;
    ll e=max(n,m);
    dig = (dig + (e - z + MOD)%MOD)%MOD;
    ll ans =modmult(y,modmult(y-1,modinv(2)));
    if (n == m)
    {
        ll oc = modmult(2,modmult(n, modmult(n - 1, modmult(n + 1, modinv(6)))));
        oc = oc % MOD;
        oc = (oc - (modmult(n, modmult(n - 1, modinv(2)))) + MOD)%MOD;
        oc = modmult(oc, 2);
        cout << (ans  - oc + MOD) % MOD << endl;
        done;
    }
    else
    {
        ll w = (dig - (2*(z - 1)) + MOD)%MOD;
        ll oc = modmult(2,modmult(z, modmult(z - 1, modmult(z + 1, modinv(6)))));
        oc = (oc + modmult(w, modmult(z, modmult(z - 1, modinv(2)))))%MOD;
        oc = oc % MOD;
        oc = modmult(oc, 2);
        cout << (ans - oc + MOD) % MOD << endl;
        done;
