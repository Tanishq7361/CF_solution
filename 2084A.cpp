...
void idharDekh()
{
    ...
    if(ct>=1 && n%2==0){
        ...
        if(n&1){
            if(a[n/2]!=ans){
                ans2++;
                outm.pb({n/2,mpp3[ans]-1});
                swap(a[n/2],a[mpp3[ans]-1]);
                swap(b[n/2],b[mpp3[ans]-1]);
                ll temp=mpp3[a[n/2]];
                ll temp2=mpp4[b[n/2]];
                mpp3[a[n/2]]=mpp3[ans];
                mpp3[ans]=temp;
            ...
            fr(i,0,n/2){
                if(a[i]==b[n-1-i]){
                    ...
                    ans2++;
                    outm.pb({mpp4[a[i]]-1,n-1-i});
                    swap(a[n-1-i],a[mpp4[a[i]]-1]);
                    swap(b[n-1-i],b[mpp4[a[i]]-1]);
                    ll temp=mpp3[a[n-1-i]];
                    ll temp2=mpp4[b[n-1-i]];
                    mpp3[a[n-1-i]]=mpp3[a[mpp4[a[i]]-1]];
                    mpp3[a[mpp4[a[i]]-1]]=temp;
                    mpp4[b[n-1-i]]=mpp4[a[mpp4[a[i]]-1]];
                    mpp4[a[mpp4[a[i]]-1]]=temp2;
                }
            }
            out(ans2);
            for(auto x: outm){
