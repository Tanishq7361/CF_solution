...
inline bool comprev(pair<ll,ll> a, pair<ll,ll> b){if(a.ss ==b.ss){return a.ff>b.ss;} else{return a.ss>b.ss;}}
...
void idharDekh()
{
    ...
    sort(all(ans),comprev);
    fr(i,0,z){
        final+=ans[i].ff;
        //cout<<final<<endl;
        final+=(ans[i].ss)*(z-i-1)*m;
        //cout<<final<<endl;
    }
    out(final);
}
